...
export interface Manifest {
   ...
   *
   * インストール先が競合している場合、インストールは失敗します。
   * 特にバンドルの展開先のディレクトリはそのディレクトリ下のすべてのファイルと競合します。
   * （例えばバンドルを`$data/some_dir/`に展開する場合、バンドルの中身にかかわらず、`$data/some_dir/hoge.txt`をインストールしようとすると競合します。）
   */
  resources: {
    /**
     * このファイルの取得元。
